...
const STATIC_IMAGES: { src: string; brightness: number }[] = [
  ...
  { src: "/images/millstadt-ems/hero/811D6F5D-4DD7-4E6C-B758-7B4167F03B38.jpeg", brightness: 0.7  },
...
export default function HeroCarousel() {
    ...
    fetch("/api/admin/media?collection=hero")
      ...
      .then((data: { url: string; brightness: number }[]) => {
        if (Array.isArray(data) && data.length > 0) {
          setImages(data.map(d => ({ src: d.url, brightness: d.brightness ?? 0.45 })));
        }
      })
  ...
  return (
    <>
      <Image src={images[slots[0]].src} alt="Millstadt EMS" fill style={{ ...imgStyle(images[slots[0]].brightness), zIndex: 0 }} priority={slots[0] === 0} unoptimized={images[slots[0]].src.startsWith("https://")} />
      <Image key={slots[1]} src={images[slots[1]].src} alt="" fill aria-hidden unoptimized={images[slots[1]].src.startsWith("https://")} style={{ ...imgStyle(images[slots[1]].brightness), zIndex: 1, opacity: showTop ? 1 : 0, transition: showTop ? "opacity 2.5s ease-in-out" : "none" }} />
    </>
  );
}
