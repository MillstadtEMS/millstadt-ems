...
- Production renders only the branded `Financial Records Request` / `Coming Soon` experience. Financial document, request, viewer, and public-990 APIs return 404 in production.
...
| High | Financial administration relies on existing shared admin authentication; MFA and dedicated least-privilege approver roles are not proven. | Account compromise could approve/release records. Require named accounts, MFA, role checks, session rotation/expiry, and reauthentication for release actions. | Millstadt identity owner; date unassigned, mandatory before `GO LIVE`. | Yes |
| High | Upload security has no approved malware-scanning service and stores bytes in process memory. | A crafted allowed file could reach staff. Add private object storage, quarantine, scanner verdict, safe download headers, and time-limited authorization. | Millstadt storage/security owner; date unassigned, mandatory before `GO LIVE`. | Yes |
