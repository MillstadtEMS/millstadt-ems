...
writeFileSync(output,JSON.stringify({baseline:'381e0e84e4838dfa398daef6cc8c55437741cd0b',previewOnly:true,documentCount:rows.length,sourceReconciliation:'All 88 document mappings reconciled. All 22 tax sheets visually checked against the 064 AMBULANCE row and Total + Overlap Rate Setting EAV. All 66 local PDFs served byte-for-byte.',liveBinding:{component:'app/financials-information-hub/AnnualCallSummary.tsx',endpoint:'/api/cad/log',refreshMs:60000,status:'Preserved; local database configuration unavailable'},documents:rows,artwork:{canonicalPath:resolve(artPublic),originalSource:artSource,role:'civic-resource artwork',title:'Get Out & Vote',width:1536,height:1024,sha256:sha(readFileSync(artPublic)),unchanged:true}},null,2));
console.log(`Reconciled ${rows.length} documents, 66 local PDF responses, and unchanged artwork.`);
}
void main();
