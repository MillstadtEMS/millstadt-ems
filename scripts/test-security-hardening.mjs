...
pass("site shell preserves the ticker and clears transient call details after hover or navigation");

const dependencies = JSON.parse(await source("package.json"));
assert.ok(dependencies.dependencies.zod);
assert.equal(dependencies.dependencies.next, "16.3.1");
assert.equal(dependencies.dependencies.nodemailer, "^9.0.5");
...
pass("dependency scanning is automated and patched framework and mail dependencies are pinned");
