...
| Required fields | Full name, email address, mailing address, city, state, ZIP code, one or more restricted document IDs, the exact terms acknowledgment, signature full name, valid signature, and exact final electronic-submission confirmation. Validation: `lib/financials-hub/dev-store.ts` (`validateRequestInput`). |
...
| Validation | Required-value checks, email format, catalog allowlist, exact acknowledgment/action/final-confirmation text and booleans, signature-full-name equality, typed-name equality, PNG signature structure/size, request source, CSRF, body limits, rate limit, and idempotency. Sources: route and store files above. |
| Signature | Required. The requester enters the full name to place on the request record, then supplies a drawn PNG or typed signature. Applying the signature reveals a separate unchecked final confirmation; both are included in the server-generated signed request PDF. Sources: `FinancialsSignaturePad.tsx`, `FinancialsArchivePrototype.tsx`, `dev-store.ts`, `agreement-pdf.ts`. |
...
| Subject | `[Millstadt EMS] Restricted document request {request ID}`. Source: `notifyFinancialsHubAdmins`. |
...
| Attachments | Signed request PDF when generation succeeds. Source: `signedAgreementForRequest` and `notifyFinancialsHubAdmins`. |
...
| Required fields | Reporter full name, email, referenced document, page/section/location, concern category, specific concern, the exact good-faith certification, and a valid signature. Sources: route and `validateAccuracyInput` in `accuracy-store.ts`. |
