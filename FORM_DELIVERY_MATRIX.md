...
| Optional fields | Address line 2 is accepted by the server but is not currently displayed in the public form. Source: `CreateRequestInput` in `lib/financials-hub/dev-store.ts`. |
...
| Initial admin recipient | Server-only test/production recipient group configured by environment. Recipient addresses are intentionally not stored in source. In local and automated testing, outbound delivery is disabled unless a non-production test sink is explicitly configured. Source: `lib/financials-hub/config.ts`. |
...
| Requester confirmation | On-page confirmation and status refresh only. No requester email is implemented or authorized in the current source. |
...
| --- | --- | --- | --- |
| Approve access | `POST /api/admin/financials/access-requests/{id}/approve` | Authorized admin; current pending/under-review request; matching request version; selected requested documents; unchanged document versions | Records approval and expiration; enables later controlled-viewer authorization. |
| Deny access | `POST /api/admin/financials/access-requests/{id}/deny` | Authorized admin; current pending/under-review request; matching request version | Records denial; no document access. |
| Revoke access | `POST /api/admin/financials/access-requests/{id}/revoke` | Authorized admin; current approved request; matching request version | Removes approved IDs and invalidates active viewer sessions. |
| Expire access | `POST /api/admin/financials/access-requests/{id}/expire` | Authorized admin; current approved request; matching request version | Expires access and invalidates active viewer sessions. |
