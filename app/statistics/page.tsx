    ...
    <main style={{ maxWidth: 1040, margin: "0 auto", padding: "8px 18px 64px" }}>
      ...
      {loading && !data ? (
        ...
        <>
          ...
          <div style={grid}>
            <GroupCard title="Trauma" group={data.groups.trauma} accent="#fca5a5" total={data.total} calls={calls} />
            <GroupCard title="Medical" group={data.groups.medical} accent="#86efac" total={data.total} calls={calls} />
            <GroupCard title="Uncategorized" group={data.groups.uncategorized} accent="#94a3b8" total={data.total} calls={calls} />
...
function GroupCard({ title, group, accent, total, calls }: { title: string; group: ClassGroup; accent: string; total: number; calls: PubCall[] }) {
  return (
    <Card>
      ...
      {group.categories.length === 0 ? <span style={{ color: "#475569", fontSize: 12 }}>None.</span> :
        group.categories.map((c) => <Row key={c.name} label={c.name} count={c.count} pct={c.pct} accent={accent} calls={calls.filter((x) => x.category === c.name)} />)}
    </Card>
  );
}
