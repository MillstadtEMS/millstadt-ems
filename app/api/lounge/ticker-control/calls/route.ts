...
function rowToCall(row: CadCallRow) {
  return {
    ...
    structured: {
      ...
      fireResponded: !!row.fire_responded,
      fireAgencies: Array.isArray(row.fire_agencies) ? row.fire_agencies : [],
      policeResponded: !!row.police_responded,
      policeAgencies: Array.isArray(row.police_agencies) ? row.police_agencies : [],
      emsMutualAid: !!row.ems_mutual_aid,
      emsMutualAidAgencies: Array.isArray(row.ems_mutual_aid_agencies) ? row.ems_mutual_aid_agencies : [],
      unitDispositions: (row.unit_dispositions && typeof row.unit_dispositions === "object" && !Array.isArray(row.unit_dispositions)) ? row.unit_dispositions : {},
    },
  };
}
