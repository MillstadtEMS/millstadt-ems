  ...
  if (form.share.emailEmployee && emp.email) {
    try {
      await sendEmployeeEmail({
        ...
        bodyText: `A finalized copy of the ${spec.label} is available in the authenticated Employee Lounge.`,
  ...
  if (form.share.emailAdminInbox) {
    try {
      await emailAdmins({
        ...
        meta: `Document ID ${form.id.slice(0,8)}`,
        bodyText: `A finalized copy of the ${spec.label} is available in the authenticated administrator portal.`,
        link: { url: protectedPdfUrl, label: "Open PDF" },
        subject: `Millstadt EMS - ${spec.label} - ${employeeName} - ${new Date().toISOString().slice(0,10)}`,
      });
      emailedAdmin = true;
