  ...
  if (form.share.emailEmployee && emp.email) {
    try {
      await sendEmployeeEmail({
        ...
        bodyText: `A finalized copy of the ${spec.label} for ${employeeName} is available in the authenticated Employee Lounge.`,
  ...
  if (form.share.emailAdminInbox) {
    try {
      await emailAdmins({
        ...
        bodyText: `A finalized copy of the ${spec.label} for ${employeeName} is available in the authenticated administrator portal.`,
        link: { url: protectedPdfUrl, label: "Open PDF" },
        subject: `Millstadt EMS - ${spec.label} - ${employeeName} - ${new Date().toISOString().slice(0,10)}`,
      });
      emailedAdmin = true;
