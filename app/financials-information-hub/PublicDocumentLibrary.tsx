...
export default function PublicDocumentLibrary({
  ...
  const financialDocuments = filteredDocuments.filter(
    (document) => document.kind === "financial_document",
  );
    ...
    <main className={styles.page}>
      {isDevelopmentPreview && (
        <div className={styles.previewBanner} role="status">
          Local development preview · Sample records only
        </div>
      )}
          ...
          {filteredDocuments.length === 0 ? (
            ...
            <div className={styles.libraryGroups}>
              ...
              {financialDocuments.length > 0 && (
                <DocumentGroup
                  title="Financial reports & records"
                  description="Published financial reports, budgets, audits, and related public records."
  ...
  return (
    <article className={styles.documentCard}>
      ...
      <dl className={styles.metadata}>
        ...
        <div><dt>Date</dt><dd>{document.dateLabel}</dd></div>
      ...
      <div className={styles.cardActions}>
        <a
          ...
          data-analytics-document-kind={document.kind === "form_990" ? "public_form_990" : "public_financial_document"}
          data-analytics-document-id={document.id}
        >
          <ZoomIn aria-hidden="true" /> Open &amp; enlarge
        </a>
        <a
...
function documentReportHref(document: PublicLibraryDocument) {
  ...
  const body = [
    ...
    `Link: ${PUBLIC_SITE_ORIGIN}${document.viewUrl}`,
    "",
